...
const SalesByPerformer = ({
  ...
  const [topPerformers, setTopPerformers] = useState(20);

  const noDataHandler = () => {
    if (isSalesByPerformerFetching) return <Spinner spinnerTime={false} />;
  ...
  const APICall = (limit, sortby) => {
    ...
    fetchSalesByPerformerRequest({
      ...
      limit: limit ? limit : topPerformers
    });
    // setStartOnLoad(false)
  };
  ...
  const getDateTime = () => {
    return SALESDATE(
      salesByPerformer.performerSaleProfitList,
      moment(start).format("MM/DD/YYYY"),
      moment(end).format("MM/DD/YYYY")
    );
  };

  const getTotalSales = performerId => {
    const chartData = [];
    let labelArray = [];
    labelArray = SALESDATE(salesByPerformer.performerSaleProfitList);
    labelArray.forEach(element => {
      let result = "",
        totalSaleby = 0,
        totalProfitby = 0,
        isPerformerExist = false;

      result = salesByPerformer.performerSaleProfitList.filter(
        log => log.time === element
      );

      isPerformerExist = result[0]
        ? result[0].performers.find(
            performer => performer.performerId == performerId
          )
        : false;

      if (result.length > 0 && isPerformerExist) {
        result[0].performers.map(performer => {
          if (performer.performerId == performerId) {
            sortBy == "sale"
              ? (totalSaleby = performer.totalSale)
              : (totalProfitby = performer.totalProfit);
          }
        });
    ...
    // console.log(sortBy, "performerId :", performerId, chartData)
    return chartData;
  };
  ...
  const getDataSets = () => {
    ...
    salesByPerformer.topPerformerList.forEach(e => {
      topPerformerList.push({ name: e.performerName, id: e._id });
    });

    // console.log("topPerformerList:", topPerformerList)
    topPerformerList.forEach((performer, i) => {
      let dataObj = {};
      dataObj.label = performer.name;
      dataObj.fill = false;
      ...
      dataObj.data = getTotalSales(performer.id);
      ...
      <div className="row inner_box_body padL3T5">
        ...
        <div className="col-4 fl_eq_box">
          <Form.Control
            ...
            value={topPerformers}
            onChange={evt => {
              setTopPerformers(evt.target.value);
              APICall(evt.target.value, undefined);
            }}
          >
            <option value="">Select Top Performers</option>
        ...
        <div className="col-4 fl_eq_box">
          ...
          >
            <option value="sale">Sale By Top Performers</option>
            <option value="profit">Profit By Top Performers</option>
          </Form.Control>
        </div>
            ...
            <>
              <div className="tbl_main date_tbl cm_tbl_btn_main ev_queue">
                <div className="inner_tbl">
                  <BootstrapTable
                    data={salesByPerformer.topPerformerList}
                    version="4"
                    striped
                    hover
                    pagination
                ...
                <Line
                  ...
                  options={{
                    ...
                    title: {
                      ...
                      text:
                        sortBy == "sale"
                          ? "Sale By Top Performers"
                          : "Profit By Top Performers",
                      fontSize: 20
                    },
                    legend: {
                      display: true,
...
export default SalesByPerformer;
